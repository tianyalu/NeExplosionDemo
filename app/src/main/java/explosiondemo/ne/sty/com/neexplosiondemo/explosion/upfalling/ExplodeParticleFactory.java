...
import explosiondemo.ne.sty.com.neexplosiondemo.explosion.Utils;
...
public class ExplodeParticleFactory extends ParticleFactory {
    public static final int PART_WH = 14; //默认小球宽高
    ...
    private Particle generateParticle(int color, Random random) {
        ExplodeParticle particle = new ExplodeParticle(0, 0, color);
        particle.radius = V;
        ...
        particle.baseCx = f;
        particle.cx = f;
        f = mBound.exactCenterY() + (Y * (random.nextFloat() - 0.5f));
        particle.baseCy = f;
        particle.cy = f;
